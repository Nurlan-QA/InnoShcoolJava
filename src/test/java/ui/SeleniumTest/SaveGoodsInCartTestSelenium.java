...
public class SaveGoodsInCartTestSelenium extends BaseTestSelenium {
...
//        Нажмаем на кнопку корзины
        driver.findElement(By.xpath("//*[@id='open-cart-btn']")).click();

//        Проверяем, что товар ЕСТЬ в корзине ДО рефреша
        List<WebElement> cartItems = driver.findElements(By.xpath("//div[@id='cart-items']//*[text()='Кефир']"));
        Assertions.assertThat(cartItems)
                .as("Товар должен быть в корзине до рефреша")
                .isNotEmpty();
...
//        Проверяем отсутствие добавленного товара в корзине (товар не сохраняется в корзине)
        Assertions.assertThat(emptyCartItems)
                .as("Корзина должна быть пустой после рефреша")
                .isEmpty();  // Если корзина должна сохраняться (localStorage), то этот ассерт должен быть isNotEmpty!
